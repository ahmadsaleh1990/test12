 ...
 * Provides segmented control over a group of buttons, similarly to a radio group.<br>
 * <b>Note : </b>Differs from {@link me.SyncWise.Android.Widgets.SegmentedRadioGroup SegmentedRadioGroup} that multiple buttons can be selected.
 * 
 ...
 *
 */
public class SegmentedToggleGroup extends LinearLayout {
	...
	/**
	 * Builds a toggle button in order to add it to a segmented toggle group.
	 * 
	 * @param context	The application context.
	 * @return	A toggle button made ready to be added to a segmented toggle group.
	 */
	public static ToggleButton buildChild ( final Context context ) {
		// Declare and initialize a new toggle button
		ToggleButton toggleButton = new ToggleButton ( context );
		// Remove the button drawable
		toggleButton.setButtonDrawable ( new StateListDrawable () );
		// Reset the minimum width and height
		toggleButton.setMinimumHeight ( 0 );
		toggleButton.setMinimumWidth ( 0 );
		toggleButton.setMinWidth ( 0 );
		// Set the text appearance
		toggleButton.setTextAppearance ( context , R.style.TextAppearance_Medium );
		...
		toggleButton.setTextColor ( context.getResources ().getColorStateList ( R.color.radio_colors ) );
	...
	/**
	 * Gets the child position inside the group based on the child view.<br>
	 * If the provided view do not match any child in the group, -1 is returned.
	 * 
	 * @param view	View representing the child view.
	 * @return	The child position whose view matches, or -1 if none is found.
	 */
	public int getChildIndex ( final View view ) {
		// Declare and initialize an integer used to hold the child position
		int position = -1;
		// Iterate over all children in the group
		for ( int i = 0 ; i < super.getChildCount () ; i ++ )
			// Compare views
			if ( super.getChildAt ( i ).equals ( view ) ) {
				// Store the child's position
				position = i;
				break;
			} // End if
	...
	public void setText ( final int position , final String text ) {
		...
		try {
			...
			( (ToggleButton) super.getChildAt ( position ) ).setText ( text );
	 ...
	 */
	public SegmentedToggleGroup ( Context context , AttributeSet attrs) {
		// Superclass method call
		super ( context , attrs );
	...
	protected void onFinishInflate () {
		...
		// Finalize inflating a segmented toggle group by changing some of the buttons properties
		finalizeButtons ();
	}
	...
	public void notifyChildViewsChanged () {
		// Finalize building a segmented toggle group by changing some of the buttons properties
		finalizeButtons ();
	}
