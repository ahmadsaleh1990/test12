 ...
 *
 */
public class SOSTrackerWatcher implements TextWatcher {
	 ...
	 */
	private WeakReference < SOSTrackerDetailActivity > activity;
	 ...
	 */
	public SOSTrackerWatcher ( final SOSTrackerDetailActivity activity , final View orderView , final SOSTracker tracker , final int type ) {
		// Initialize variables
		this.activity = new WeakReference < SOSTrackerDetailActivity > ( activity );
	...
	public void afterTextChanged ( Editable s ) {
		...
		case Type.SHARE_OF_SHELF:
			tracker.setBDAMeter  ( getNumber ( s.toString () ) );
			break;
		...
		try {
			...
			if ( trackerView != null && trackerView.get () != null && activity != null && activity.get () != null ) {
				...
				SOSTrackerDetailsAdapter.refreshOrderView ( activity.get () , trackerView.get ()  );
