 ...
 *
 */
public class TrackerWatcher implements TextWatcher {
	...
	/**
	 * Weak reference to the {@link me.SyncWise.Android.Modules.ShareOfShelf.ShareOfShelfTrackerDetailsActivity ShareOfShelfTrackerDetailsActivity} screen.
	 */
	private WeakReference < ShareOfShelfTrackerDetailsActivity > activity;
	 ...
	 * Constructor.
	 * 
	 * @param activity	Reference to the {@link me.SyncWise.Android.Modules.ShareOfShelf.ShareOfShelfTrackerDetailsActivity ShareOfShelfTrackerDetailsActivity} screen.
	 ...
	 */
	public TrackerWatcher ( final ShareOfShelfTrackerDetailsActivity activity , final View orderView , final Tracker tracker , final int type ) {
		// Initialize variables
		this.activity = new WeakReference < ShareOfShelfTrackerDetailsActivity > ( activity );
	...
	public void afterTextChanged ( Editable s ) {
		...
		case Type.SHARE_OF_SHELF:
			...
			break;
		case Type.COMPETITOR_SHARE_OF_SHELF:
			tracker.setCurrentCompetitorShareOfShelf ( getNumber ( s.toString () ) );
			break;
		case Type.CATEGORY:
			tracker.setCurrentCategory ( getNumber ( s.toString () ) );
			break;
		default:
			// Do nothing
			return;
		...
		try {
			...
			if ( trackerView != null && trackerView.get () != null && activity != null && activity.get () != null ) {
				...
				ShareOfShelfTrackerDetailsAdapter.refreshOrderView ( activity.get () , trackerView.get () , activity.get ().getDecimalFormat () );
