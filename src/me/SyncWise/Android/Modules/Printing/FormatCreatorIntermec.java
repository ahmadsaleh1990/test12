...
public class FormatCreatorIntermec implements FormatCreator {
	...
	public FormatCreatorIntermec ( Context context , TransactionHeaders transactionHeaders , int type , int printout ) {
	...
	//client stock count
	public FormatCreatorIntermec ( Context context , ClientStockCountHeaders  clientStockCountHeaders , int type , int printout ) {
	...
	//VehiclesStock
	public FormatCreatorIntermec ( Context context , ArrayList < vehiclesStockPrinting> vehiclesStock , int type , int printout ) {
	...
	//Plan visit report
	public FormatCreatorIntermec ( Context context   , ArrayList < CycleCalls> cycleCalls , int type,int printout ,String a  ) {
	...
	//COLLECTIONSUMMARY
	public FormatCreatorIntermec ( Context context , int type , ArrayList <CollectionSummaryPrinting> collectionSummaryHeaderss ,ArrayList <SummaryReturnPrinting> summaryReturnPrinting , int printout ) {
	...
	public FormatCreatorIntermec ( Context context, ArrayList <LoadInSummaryPrinting> loadInSummaryPrinting  , int type ,int i,int i1  ) {
	...
	public FormatCreatorIntermec (  int type , ArrayList <CollectionHeaders> collectionHeaderss ,Context context , int printout ) {
	...
	//INVOICESUMMARY 
	public FormatCreatorIntermec ( Context context , int type ,   int printout , ArrayList < TransactionHeaders> transactionHeaderss) {
	...
	//clientAssetPrinting
	public FormatCreatorIntermec ( Context context   , ArrayList <ClientAssetPrinting>  clientAssetPrinting, int type) {
	...
	//missed items
	public FormatCreatorIntermec ( Context context , int type  , ArrayList <MissedItems>  missedItems) {
	...
	//item Distribution
	public FormatCreatorIntermec ( Context context , ArrayList <Items>  items  , ArrayList <ItemDistrbutionPrinting>  itemDistrbutionPrinting, int type , int printout ) {
		this.items = items;
	...
	public FormatCreatorIntermec ( Context context , MovementHeaders movementHeaders , ArrayList<MovementDetails> details , int type , int printout ) {
	...
	//receipt
	public FormatCreatorIntermec ( Context context , CollectionHeaders collectionHeaders , int type , int printout ) {
...
//						.where (CollectionHeadersDao.Properties.CollectionCode.eq ( collectionHeaders.getCollectionCode () ) , CollectionHeadersDao.Properties.ParentCode.eq ( collectionHeaders.getCollectionCode () )  ).unique ())   ).list ();
	...
	public String Creator ( int type , int copyType ) {
		...
		if ( type == Type.INVOICE   ||    type==Type.RETURN || type == Type.INVOICE_DISCOUNT ) {
	     ...
	     if((transactionHeader.getInfo1() .equals( "") && transactionHeader.getInfo2().equals( "")))
 	        		   ...
 	        		   ArrayList < SummaryReturnPrinting > summaryReturnPrintingCredit= new ArrayList < SummaryReturnPrinting > ();
	        			  ...
	        			  for( int i = 0 ; i < summaryReturnPrintings.size () ; i ++ ) {
	        				  SummaryReturnPrinting    tr = summaryReturnPrintings.get ( i );
						         if( tr.getClientType() == 1 || (tr.getInfo1()!=null && tr.getInfo1()=="1") ) 
...
//					  	 
			...
			if ( type == Type.COLLECTIONSUMMARY   ) {
		        int lastPageM=collectionSummaryHeaderss.size() / 36;
				int restM = collectionSummaryHeaderss.size() % 36;
				footer = new StringBuilder ();
				 
				 if(pageNumber == ( lastPageM + 1 )    )
			    {
					 if(pageNumber==1)
		  	  				if(( 36 - collectionSummaryHeaderss.size() - 3 )>0)
		  				{	    for(int u=0 ; u< 38-collectionSummaryHeaderss.size()  - 3 ;u++)
		  					footer.append ( NEWLINE );
		  				}
		  	  				else{
				 if(restM>4)
...
//	 		} // End if
		      ...
		      body.append ("              TOTAL CASH AND CREDIT INVOICE:                "+moneyFormat.format(totalCreditInvoice+totalCashCreditInvoice+totalCashInvoice)+    NEWLINE);
