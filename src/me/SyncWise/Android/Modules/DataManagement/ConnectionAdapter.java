...
/**
 * A concrete BaseAdapter that is backed by an array of {@link me.SyncWise.Android.Database.ConnectionSettings ConnectionSettings} objects.
 * 
 * @author Elias
 *
 */
public class ConnectionAdapter extends ArrayAdapter < ConnectionSettings > {
	 ...
	 * @param connections	List of {@link me.SyncWise.Android.Database.ConnectionSettings ConnectionSettings} objects.
	 */
	public ConnectionAdapter ( Context context, int layout , List < ConnectionSettings > connections ) {
		// Superclass method call
		super ( context , layout , connections );
	...
	public View getView ( int position , View convertView , ViewGroup parent ) {
		...
		SpannableString connection = new SpannableString ( getItem ( position ).getConnectionSettingDescription () + " " + getItem ( position ).getConnectionSettingURL () );
		// Apply style span
		connection.setSpan ( new StyleSpan ( Typeface.BOLD ) , 0 , getItem ( position ).getConnectionSettingDescription ().length () , 0 );
		// Apply color span
		connection.setSpan ( new ForegroundColorSpan ( colorDeepSkyBlue ) , 0 , getItem ( position ).getConnectionSettingDescription ().length () , 0 );
	...
	public View getDropDownView ( int position , View convertView , ViewGroup parent ) {
		...
		SpannableString connection = new SpannableString ( index + getItem ( position ).getConnectionSettingDescription () + " " + getItem ( position ).getConnectionSettingURL () );
		// Apply style span
		connection.setSpan ( new StyleSpan ( Typeface.BOLD ) , 0 , index.length () + getItem ( position ).getConnectionSettingDescription ().length () , 0 );
		// Apply color span
		connection.setSpan ( new ForegroundColorSpan ( colorDeepSkyBlue ) , index.length () , index.length () + getItem ( position ).getConnectionSettingDescription ().length () , 0 );
		// Set the reason label
		viewHolder.label.setText ( connection , BufferType.SPANNABLE );
