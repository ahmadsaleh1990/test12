 ...
 *
 */
public class BasketPromotionsRFRAdapter extends BaseExpandableListAdapter {
	 ...
	 */
	private final List < BasketPromotionRFR > basketPromotions;
	 ...
	 */
	public BasketPromotionsRFRAdapter ( final Context context , final int groupLayout , final int childLayout , final List < BasketPromotionRFR > basketPromotions ) {
	...
	public int getChildrenCount ( int groupPosition ) {
		try {
			...
			return ( (BasketPromotionRFR) getGroup ( groupPosition ) ).getPromotionDetails ().size ();
		} catch ( Exception exception ) {
			// Invalid group position
			return 0;
		} // End of try-catch block
	...
	public Object getChild ( int groupPosition , int childPosition ) {
		try {
			...
			return ( (BasketPromotionRFR) getGroup ( groupPosition ) ).getPromotionDetails ().get ( childPosition );
		} catch ( Exception exception ) {
			// Invalid group position
			return null;
		} // End of try-catch block
	 ...
	 */
	public OrderRFR getChildInvoiceReference ( int groupPosition , int childPosition ) {
		try {
			// Return the invoice for the current promotion detail of the basket at the indicated position
			return ( (BasketPromotionRFR) getGroup ( groupPosition ) ).getInvoices ().get ( childPosition );
		} catch ( Exception exception ) {
			// Invalid group position
			return null;
		} // End of try-catch block
	...
	public View getGroupView ( int groupPosition , boolean isExpanded , View convertView , ViewGroup parent ) {
		...
		BasketPromotionRFR basketPromotion = (BasketPromotionRFR) getGroup ( groupPosition );
		...
		if(isExpanded)
			basketPromotion.setExpanded(true);	
		else
			basketPromotion.setExpanded(false);
		// Return the view
		return convertView;
	}
