...
public class DirectReturnDetailAdapter extends ArrayAdapter < OrderDirectReturn > implements OnClickListener , OnFocusChangeListener {
	...
	public static class ViewHolder {
		public OrderDirectReturn order;
		public int position;
	 ...
	 */
	public DirectReturnDetailAdapter ( Context context , int layout , List < OrderDirectReturn > orderItems , final Currencies currency , final boolean itemsEnabled , final boolean usePicker ) {
	 ...
	 * @param currency	{@link me.SyncWise.Android.Database.Currencies Currencies} object hosting the current sales order currency.
	 */
	private DirectReturnDetailAdapter ( Context context , int layout , List < OrderDirectReturn > orderItems , final Currencies currency ) {
	...
	public static void refreshOrderView ( final Context context , final View view , final boolean usePicker , final DecimalFormat _moneyFormat , final Currencies currency , final String _codeLabel , final String _totalLabel , final String _reasonLabel , final String _expiryDateLabel , final String _newLine , final Integer _brownColor ) {
		...
		if ( ! viewHolder.order.getReasonOfReturn ().isEmpty () && ! usePicker ) {
			...
			double _totalAmount = netValue + taxesAmount;
			...
			viewHolder.code.setText ( TextUtils.concat ( code , newLine , totalAmount , newLine , reason , newLine , expiryDate ) , BufferType.SPANNABLE );

		} // End if
		else
			// Either the current item has no reasons or number pickers are disabled
			// Simply display the item code
			viewHolder.code.setText ( code , BufferType.SPANNABLE );
	}
	...
	public View getView ( int position , View convertView , ViewGroup parent ) {
		try {
			...
			OrderDiretReturnWatcher orderWatcher = null;
			
			// Refresh the order view
			refreshOrderView ( getContext () , convertView , usePicker , moneyFormat , currency , codeLabel , totalLabel , reasonLabel , expiryDateLabel , newLine , brownColor );
			...
			if ( ItemsUtils.isBig ( getItem ( position ).getItem () ) ) {
				...
				if ( usePicker ) {
					...
					if ( DirectReturnDetailsActivity.selectedReasonCode == null )
						...
						viewHolder.pickerBig.setNumber ( getItem ( position ).getQuantityBig ( DirectReturnDetailsActivity.selectedReasonCode ) );
					// Assign a new text watcher
					orderWatcher = new OrderDiretReturnWatcher ( (DirectReturnDetailsActivity) getContext () , convertView , getItem ( position ) , OrderDiretReturnWatcher.Type.QUANTITY_BIG );
					...
					if ( getItem ( position ).getItem ().getItemCode ().equals ( DirectReturnDetailsActivity.selectedItemCode ) && DirectReturnDetailsActivity.selectedReasonCode != null && DirectReturnDetailsActivity.expiryDateEdit != null ) 
						// Make sure the big UOM layout is visible
						viewHolder.big.setVisibility ( View.VISIBLE );
					else
			...
			if ( ItemsUtils.isMedium ( getItem ( position ).getItem () ) ) {
				...
				if ( usePicker ) {
					...
					if ( DirectReturnDetailsActivity.selectedReasonCode == null )
						...
						viewHolder.pickerMedium.setNumber ( getItem ( position ).getQuantityMedium ( DirectReturnDetailsActivity.selectedReasonCode ) );
					// Assign a new text watcher
					orderWatcher = new OrderDiretReturnWatcher ( (DirectReturnDetailsActivity) getContext () , convertView , getItem ( position ) , OrderDiretReturnWatcher.Type.QUANTITY_MEDIUM );
			...
			if ( usePicker ) {
				...
				if( DirectReturnDetailsActivity.selectedReasonCode == null)
					...
					viewHolder.pickerSmall.setNumber(getItem ( position ).getQuantitySmall ( DirectReturnDetailsActivity.selectedReasonCode ) );
				// Assign a new text watcher
				orderWatcher = new OrderDiretReturnWatcher ( (DirectReturnDetailsActivity) getContext () , convertView , getItem ( position ) , OrderDiretReturnWatcher.Type.QUANTITY_SMALL );
			...
			if(  usePicker && DirectReturnDetailsActivity.selectedItemCode != null &&viewHolder.order.getItem ().getItemCode ().equals ( DirectReturnDetailsActivity.selectedItemCode ) )
	...
	public void onClick ( View view ) {
		...
		( (DirectReturnDetailsActivity) getContext () ).selectedView ( view , ! usePicker , true );
	}
