...
/**
 * A concrete BaseAdapter that is backed by an array of {@link me.SyncWise.Android.Modules.Return.Return Return} objects.
 * 
 * @author Elias
 *
 */
public class ReturnDetailsAdapter extends ArrayAdapter < Return > {
	 ...
	 */
	public ReturnDetailsAdapter ( Context context , int layout , List < Return > returnItems , final int currencyRounding , final boolean itemsEnabled ) {
		// Overloaded constructor
		this ( context , layout , returnItems , currencyRounding );
	 ...
	 * @param currencyRounding	Integer used to indicate the currency rounding.
	 */
	public ReturnDetailsAdapter ( Context context , int layout , List < Return > returnItems , final int currencyRounding ) {
		// Superclass method call
		super ( context , layout , returnItems );
		// Initialize the attributes
	...
	public View getView ( int position , View convertView , ViewGroup parent ) {
		...
		// Check if the view mode is disabled
		if ( itemsEnabled ) {
		...
		SpannableString suggestedExpiryDate = new SpannableString ( suggestedLabel + " " + expiryDateLabel + ( getItem ( position ).getSuggestedExpiryDate () == null ? notAvailabelLabel : DateTime.getBriefDate ( getContext () , getItem ( position ).getSuggestedExpiryDate () ) ) );
		// Apply style span
		...
		SpannableString reason = new SpannableString ( reasonLabel + ( getItem ( position ).getReason () == null ? notAvailabelLabel : getItem ( position ).getReason ().getReasonName () ) );
		// Apply style span
		...
		SpannableString suggestedReason = new SpannableString ( suggestedLabel + " " + reasonLabel + ( getItem ( position ).getSuggestedReason () == null ? notAvailabelLabel : getItem ( position ).getSuggestedReason ().getReasonName () ) );
		// Apply style span
		...
		if ( ItemsUtils.isMedium ( getItem ( position ).getItem () ) && ItemsUtils.isRegular ( getItem ( position ).getItem () ) ) {
			...
			viewHolder.stockMedium.setText ( suggestedLabel + " : " + getItem ( position ).getSuggestedMedium () );
		} // End if
		else
		...
		if ( ItemsUtils.isRegular ( getItem ( position ).getItem () ) ) {
			...
			viewHolder.stockSmall.setText ( suggestedLabel + " : " + getItem ( position ).getSuggestedSmall () );
		} // End if
		else
