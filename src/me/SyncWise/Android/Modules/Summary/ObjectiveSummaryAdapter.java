...
public class ObjectiveSummaryAdapter extends CursorAdapter{
...
//	private final String blankVisitContactsCountLabel;
	 ...
	 */
	public ObjectiveSummaryAdapter ( Context context , Cursor cursor , int layout ) {
	...
	//	blankVisitContactsCountLabel = AppResources.getInstance ( context ).getString ( context , R.string.visit_note_label );
		...
		noteLabel=AppResources.getInstance ( context ).getString ( context , R.string.note_objective_contacts_label );
	}
...
//		// Retrieve the blank visit contacts count
 		String note = cursor.getString ( cursor.getColumnIndex (ObjectivesDao.Properties.ObjectiveDescription.columnName) );
        int  type= cursor.getInt ( cursor.getColumnIndex (ObjectiveAchievementsDao.Properties.ObjectiveAchievement.columnName) );
...
//		// Apply foreground color span
 //		contactsCount.setSpan ( new ForegroundColorSpan ( brownColor ) , label.length () , contactsCount.length () , 0 );
        String t=(type==1?" Completed ":" Canceled ") ;
 		SpannableString contactsCount1 = new SpannableString ( noteLabel + note + newLine +t);
//		// Apply foreground color span
 		contactsCount1.setSpan ( new ForegroundColorSpan ( brownColor ) , noteLabel.length () , contactsCount1.length () , 0 );
 	if(type==1)
 		contactsCount1.setSpan ( new ForegroundColorSpan ( redColor ) , noteLabel.length ()+ note.length() , contactsCount1.length () , 0 );
 	else
 		contactsCount1.setSpan ( new ForegroundColorSpan ( greenColor ) , noteLabel.length () +note.length(), contactsCount1.length () , 0 );
