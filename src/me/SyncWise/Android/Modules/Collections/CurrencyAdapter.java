...
/**
 * A concrete BaseAdapter that is backed by an array of {@link me.SyncWise.Android.Database.Currencies Currencies} objects.
 * 
 * @author Elias
 *
 */
public class CurrencyAdapter extends ArrayAdapter < Currencies > {
	 ...
	 * @param currencies	List of {@link me.SyncWise.Android.Database.Currencies Currencies} objects.
	 */
	public CurrencyAdapter ( Context context, int layout , List < Currencies > currencies ) {
		// Superclass method call
		super ( context , layout , currencies );
	...
	public View getView ( int position , View convertView , ViewGroup parent ) {
		...
		SpannableString connection = new SpannableString ( getItem ( position ).getCurrencySymbol () + " - " + getItem ( position ).getCurrencyName () );
		// Apply style span
		connection.setSpan ( new StyleSpan ( Typeface.BOLD ) , 0 , getItem ( position ).getCurrencySymbol ().length () , 0 );
		// Apply color span
		connection.setSpan ( new ForegroundColorSpan ( colorDeepSkyBlue ) , 0 , getItem ( position ).getCurrencySymbol ().length () , 0 );
	...
	public View getDropDownView ( int position , View convertView , ViewGroup parent ) {
		...
		SpannableString connection = new SpannableString ( index + getItem ( position ).getCurrencySymbol () + " " + getItem ( position ).getCurrencyName () );
		// Apply style span
		connection.setSpan ( new StyleSpan ( Typeface.BOLD ) , 0 , index.length () + getItem ( position ).getCurrencySymbol ().length () , 0 );
		// Apply color span
		connection.setSpan ( new ForegroundColorSpan ( colorDeepSkyBlue ) , index.length () , index.length () + getItem ( position ).getCurrencySymbol ().length () , 0 );
		// Set the reason label
		viewHolder.label.setText ( connection , BufferType.SPANNABLE );
