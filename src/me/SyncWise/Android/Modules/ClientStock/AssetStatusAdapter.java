...
/**
 * A concrete BaseAdapter that is backed by an array of {@link me.SyncWise.Android.Database.AssetsStatus AssetsStatus} objects.
 * 
 * @author Elias
 *
 */
public class AssetStatusAdapter extends ArrayAdapter < AssetsStatus > {
	 ...
	 */
	public AssetStatusAdapter ( Context context, int layout , List < AssetsStatus > reasons ) {
		// Superclass method call
	...
	public View getView ( int position , View convertView , ViewGroup parent ) {
		...
		// Build a spannable string out of the reason description and URL in order to apply various spans
		SpannableString reason = new SpannableString ( getItem ( position ).getStatusName () );
		// Apply style span
		reason.setSpan ( new StyleSpan ( Typeface.BOLD ) , 0 , reason.length () , 0 );
		// Apply color span
		reason.setSpan ( new ForegroundColorSpan ( colorDeepSkyBlue ) , 0 , reason.length () , 0 );
		// Set the reason label
		viewHolder.label.setText ( reason , BufferType.SPANNABLE );
	...
	public View getDropDownView ( int position , View convertView , ViewGroup parent ) {
		...
		SpannableString connection = new SpannableString ( index + getItem ( position ).getStatusName () );
		// Apply style span
		connection.setSpan ( new StyleSpan ( Typeface.BOLD ) , 0 , index.length () , 0 );
		// Apply color span
		connection.setSpan ( new ForegroundColorSpan ( colorDeepSkyBlue ) , 0 , index.length () , 0 );
		// Set the reason label
